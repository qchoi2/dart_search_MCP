...
질문 형태에 따라 검색·검증 방식을 다르게 적용한다.

- 정형 개념어(상계납입, 출자전환): 사전 정의된 정밀 문구로 본문을 검색하고 원문에 해당 문구가 있으면 확정한다.
- 문장형·복합 질의: 문장을 조사·어미를 떼어낸 핵심 토큰으로 분해하고 동의어로 넓혀 검색한 뒤, 모든 개념이 한 문서 안에 함께 나올 때만(AND-of-OR 동시출현) 확정한다.
- 제목제약 질의(현재 트리거: "공개매수"): 본문 전체검색 대신 DART 제목검색(report 모드)으로 후보 풀을 관련 보고서 수백 건대로 좁히고, 본문 개념은 전부 동시출현 검증으로 확인한다.
- 관계·순서 분석(정정 전후, 공개매수→주식교환, 최종 유효본·철회): 정정표·사건 관계를 구조적으로 대조하는 전용 분석으로 처리한다.

동의어·불용어·조사·제목제약 규칙은 `app/rules/search_terms.yaml`에 데이터로 두고 `app/rules/validation.py`가 스키마를 검증한다.
