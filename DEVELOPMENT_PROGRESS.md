...
- 배치 원문 다운로드는 최대 동시성 3으로 제한한다. HTTP rate limit·일시 네트워크 장애는 3→2→1로 감속하며 OpenDART `020`은 재시도 없이 체크포인트를 남기고 중단한다. hard deadline 제한 timeout은 채널 장애나 감속 사유로 누적하지 않는다. 대화형 검색의 실행계약은 바꾸지 않았다.
...
- 단계 4 fixture·manifest와 전용 회귀 12개를 포함한 전체 자동 테스트 132개가 통과했다. 고정 평가 24/24, compileall, diff check도 통과했으며 라이브 네트워크·90초 성능 통과 주장은 하지 않는다.
